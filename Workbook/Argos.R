...
# Error Percentage --------------------------------------------------------
  ...
  getErrorEst <- function(x){
    #Split argos tracks by individual 
    argosTracks <- x %>% 
      group_by(ID) 
    
    #Find the distribution of errors
    tab <-  argosTracks %>% 
      dplyr::select(`argos:lc`) %>% 
      table() 
    
    #Find the percentage that they occur
    percentageSum <- tab %>% 
      colSums(tab) 
    
    percentageSum <- percentageSum/sum(percentageSum)
    
    #Find the sd of this percentage
    
    percentRow<- apply(tab, 2, function(x) x/sum(x))
    sdCol <- apply(percentRow, 2, sd) 
    
    #format table
    error <- c(">1500m","500m<<1500m","250m<<500m", "<250m",   "No Accuracy", "No Accuracy", "Invalid location")
    tbl <- cbind(error, sdCol, percentageSum - (sdCol*2), percentageSum, percentageSum + (sdCol*2))
    colnames(tbl) <- c("error", "SD", "lower", "mean", "upper")
    
    table <- kbl(tbl) %>% kable_styling()
...
# Time Distribution Error -------------------------------------------------
  ...
  findTimeDist <- function(x){
    
    argosTracks <- x %>% group_split(ID)
    
    avgError <- 0 
    medError <- vector()
    for(i in 1:length(argosTracks)){
      sum <- vector()
      for(j in 1:(nrow(argosTracks[[i]])-1)){
        tmp <- as.numeric(argosTracks[[i]]$timestamp[j+1]) - 
          as.numeric(argosTracks[[i]]$timestamp[j])
        sum <- c(sum,tmp)
      }
      medError <- c(medError, sum)
    }
    return(summary(medError))
  }

# Hour Distribution -------------------------------------------------------

  findHourDist <- function(x){
    
    argosTracks <- x 
    
    hour <- format(as.POSIXct(argosTracks$timestamp), format = "%H")
    
    argosInd <- cbind(argosTracks, hour)
    
    Dist <- table(argosInd[,hour])/sum(table(argosInd[,hour]))
    return(Dist)
  }  
...
# Add Error to lat and lon ------------------------------------------------
  ...
  llGPSComb <- llGPSComb %>% 
    rename(ID = 1, 
           ...
           lat = 4,
           b = 5)
  tracks <- llGPSComb[,c(1:2,4,3,5)]
  
  
  ll2utm(llGPSComb[,c(1:2,4,3,5:7)], proj1 = "utm", proj2 = "longlat")
