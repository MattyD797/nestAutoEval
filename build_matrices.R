...
build_matrices <- function(RF_prediction, season.begin, season.end, period_length, behavior_signal){}
  ...
  ## create season-long matrices ##

  RF_prediction_sub <- predicted_full %>% filter(Julian > as.numeric(format(season.begin_fmt, "%j")) & Julian < as.numeric(format(season.end_fmt, "%j")))
  

  #calculate number of times each day that a behavior was observed
  nest.beh <- RF_prediction_sub %>% group_by(id, Julian) %>% count(b)
  nest.beh <- nest.beh[nest.beh$b==as.numeric(behavior_signal),]
  
  nest.beh.final <- matrix(nrow = length(unique(nest.beh$id)), ncol = (as.numeric(format(season.end_fmt, "%j")) - as.numeric(format(season.begin_fmt, "%j"))))
  rownames(nest.beh.final) <- c(unique(nest.beh$id))
  
  # -- subscript out of bonds error -- #
  
  for(i in 1:length(nest.beh$id)){
    nest.beh.final[match(nest.beh[i,1], pull(unique(nest.beh[,1]))), as.numeric(nest.beh[i, 2])] <- as.numeric(nest.beh[i,4])
  }
  
  #create fate matrix from behaviors
  ...
  #calculate the observation by number of GPS fixes per day
  gps.fixes <- RF_prediction_sub %>% group_by(id) %>% count(Julian)
  
  gps.fixes.final <- matrix(nrow = length(unique(gps.fixes$id)), ncol = ncol(nest.beh.final))
  ...
  #fill blank matrices
  mat_fix <- gps.fixes.final
  mat_beh <- nest.beh.final
